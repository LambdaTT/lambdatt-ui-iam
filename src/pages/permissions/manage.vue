<template>
  <La1Page Title="Gerenciar Permissões" :Breadcrumb="breadcrumb">
    <La1Card Title="Permissões por Perfil" Icon="fas fa-key">
      <template #actions>
      ...
      <div class="row">
        <div class="col-12 col-md-6">
          <InputField
            ...
            v-model="profileId"
            @update:model-value="getPermissions"
          >
          </InputField>
        </div>
          ...
          <div class="row justify-end">
            <div class="col-12 col-md-4">
              <q-btn
                ...
                color="green"
                label="Criar"
                @click="showExecPermissionModal = true"
                :disable="!!profileId == false"
              >
      ...
      <template #permissions-tree>
        <div class="text-center" v-if="!!profileId == false">
          ...
          <div class="text-h6">Não há nada para mostrar.</div>
        </div>
          ...
          <div class="q-pa-md">
            ...
            <q-list class="q-mt-sm" bordered>
              <div v-for="(mod, index) in permissions" :key="index">
                <q-expansion-item
                  header-class="q-pa-md bg-grey-8 text-white text-h6"
                      ...
                      <table class="permissions-table full-width">
                        <thead>
                          <tr>
                            <th>ENTIDADE / AÇÃO</th>
                            <th>
                              <q-icon size="xs" name="fas fa-eye"></q-icon
                              ><small>&nbsp;(VER)</small>
                            </th>
                            <th>
                              <q-icon size="xs" name="fas fa-plus"></q-icon
                              ><small>&nbsp;(CRIAR)</small>
                            </th>
                            <th>
                            ...
                            <th>
                              <q-icon size="xs" name="fas fa-trash"></q-icon
                              ><small>&nbsp;(DELETAR)</small>
                            </th>
                            <th>
                              <q-icon size="xs" name="fas fa-play"></q-icon
                              ><small>&nbsp;(EXECUTAR)</small>
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr v-for="(prm, idx) in mod.permissions" :key="idx">
                            <td>
                              <span>
                                {{ prm.entity_label }}
                                <small v-if="prm.permission_type == 'C'"
                                  >({{ prm.permission_key }})</small
                                >
                              </span>
                            </td>
                            <td class="text-center">
                              <q-checkbox
                                @update:model-value="changePermission(prm)"
                                true-value="Y"
                                false-value="N"
                                v-model="prm.do_read"
                                v-if="prm.permission_type == 'E'"
                              ></q-checkbox>
                            </td>
                            <td class="text-center">
                              <q-checkbox
                            ...
                            <td class="text-center">
                              <q-checkbox
                                ...
                                true-value="Y"
                                false-value="N"
                                v-model="prm.do_execute"
                                v-if="prm.permission_type == 'C'"
                              ></q-checkbox>
                            </td>
                          </tr>
                        </tbody>
    ...
    <La1Modal
      Title="Adicionar Permissão de Execução"
      Icon="fas fa-key"
      Persistent
      v-model="showExecPermissionModal"
      :Actions="execPermissionModalActions"
      @hide="resetInput"
    >
      <div>
        <div class="q-pa-xs">
        ...
        <div class="row">
          <div class="col-12 col-md-6">
            <InputField
              ...
              :Error="inputError.id_apm_module"
              @focus="inputError.id_apm_module = false"
          ...
          <div class="col-12 col-md-6">
            <InputField
              ...
              clearable
              dense
              v-model="input.ds_title"
              :error="inputError.ds_title"
              @focus="inputError.ds_title = false"
              maxlength="40"
              :disable="!!input.id_apm_module == false"
            ></InputField>
          </div>
          <div class="col-12">
            <InputField
              ...
              :disable="!!input.id_apm_module == false"
              maxlength="40"
              v-model="input.ds_key"
              :error="inputError.ds_key"
            ...
            >
              <template #buttons>
                <q-btn
                  ...
                  icon="fas fa-sync"
                  color="primary"
                  @click="generateExecPermissionKey"
                >
